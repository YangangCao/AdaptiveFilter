%
% T-spaced equalizer; Blind equalizer, l=2.
%
%clear all, close all
...
if M==4 
    s=b(1:2:end)+j*b(2:2:end); 
elseif M==16 
    s=2*b(1:4:end)+b(2:4:end)+j*(2*b(3:4:end)+b(4:4:end));
elseif M==64 
    s=4*b(1:6:end)+2*b(2:6:end)+b(3:6:end)+...
        j*(4*b(4:6:end)+2*b(5:6:end)+b(6:6:end)); 
elseif M==256 
    s=8*b(1:8:end)+4*b(2:8:end)+2*b(3:8:end)+b(4:8:end)+...
...
x=x(1:L:end);
xi=zeros(size(x));
N=61;           % Equalizer length
w=zeros(N,1); w(floor((N+1)/2))=1; nn=1; mu1=0.001;
gamma=mean(abs(s).^4)/(mean(abs(s).^2));
for n=N:length(x)
    tdl=x(n:-1:n-N+1);
    y(n)=w'*tdl;
    w=w-mu1*conj(y(n))*(abs(y(n))^2-gamma)*tdl;
  	xi(n)=xi(n)+abs(abs(y(n)^2)-gamma)^2;
    nn=nn+1;
    if rem(nn,100)==0 
