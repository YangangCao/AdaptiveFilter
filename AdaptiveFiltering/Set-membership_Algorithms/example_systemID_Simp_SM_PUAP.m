...
%   updating.                                                                   %
%                                                                               %
%     Adaptive Algorithm used here: Simp_SM_PUAP                                %
%                                                                               %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
L           = 3;                            % data reuse factor (L=0 -> NLMS; L=1 -> BNLMS; ...)
gamma_bar   = sqrt(5 * sigma_n2);           % upper bound for the error modulus
selector    = randint(N,K,[0 1]);           % selects which filter coefficients will be updated
                                            % at each iteration
                                            % (selector must have N rows and K columns)
...
for l=1:ensemble,
    ...
    S   =   struct('gamma_bar',gamma_bar,'filterOrderNo',(N-1),'initialCoefficients',...
                   W(:,1,l),'upSelector',selector,'gamma',gamma,'memoryLength',L);
    [y,e,W(:,:,l),nUp(1,l)]  =   Simp_SM_PUAP(d,transpose(x),S);
...
fprintf('\n');
fprintf('The average number of updates performed by the Simplified SM_PUAP Algorithm was %3.2f (in a total of %i iterations) ;\n',nUp_av, K);
fprintf('Therefore, Simplified SM_PUAP Algorithm updated only %f percent of the time.\n',pUp_av);
fprintf('\n');
