...
alpha       = 0.097;                        % small positive constant (0 < alpha <=1)
initialPower= 0;                            % initial value for the power used to normalize the input signal 


%   Initializing & Allocating memory:
W       = ones(N,(K+1),ensemble);   % coefficient vector for each iteration and realization; w(0) = [1 1 1 1].'
WDCT    = zeros(N,(K+1),ensemble);  % coefficient vector in the TRANSFORMED domain
...
for l=1:ensemble,
    ...
    S   =   struct('step',mu,'filterOrderNo',(N-1),'initialCoefficients',W(:,1,l),...
                   'gamma',gamma,'alpha',alpha,'initialPower',initialPower);
    [y,e,WDCT(:,:,l),W(:,:,l)]  =   Tdomain_DCT(d,transpose(x),S);
